...
const {SimpleTransactionDBModel} = require('cryptography').transactions.simpleTransaction;

const TxInfoHashVirtualMapDBModel = require("../maps/txs/tx-info-hash/tx-info-hash-virtual-map-db-model");
const TxRevertInfoHashVirtualMapDBModel = require("../maps/txs/tx-revert-info-hash/tx-revert-info-hash-virtual-map-db-model");

const AddressHashVirtualMapDBModel = require("../maps/addresses/addresses-hash/address-hash-virtual-map-db-model");
const AddressTxHashVirtualMapDBModel = require("../maps/addresses/addresses-tx-hash/address-tx-hash-virtual-map-db-model");

const BlockHashVirtualMapDBModel = require("../maps/blocks/block-hash-map/block-hash-virtual-map-db-model");
const BlockHeightVirtualMapDBModel = require("../maps/blocks/block-height-map/block-height-virtual-map-db-model");
const AccountHashVirtualMapDBModel = require("../maps/account-hash/account-hash-virtual-map-db-model");
const TokenHashVirtualMapDBModel = require("../maps/tokens/tokens-hash/token-hash-virtual-map-db-model");
const TokenNameVirtualMapDBModel = require("../maps/tokens/tokens-name-map/token-name-virtual-map-db-model");
const TokenTickerVirtualMapDBModel = require("../maps/tokens/tokens-ticker-map/token-ticker-virtual-map-db-model");

const MAX_CHANGE_FACTOR = 2;
const MIN_CHANGE_FACTOR = 1 / MAX_CHANGE_FACTOR;

const {BaseChainDataDBSchemaBuilt} = require('./base-chain-data-db-schema-build')

module.exports = class BaseChainDataDBModel extends DBModel {

    constructor(scope, schema = BaseChainDataDBSchemaBuilt, data, type , creationOptions){
    ...
    get _addressHashMapClass(){
        return AddressHashVirtualMapDBModel;
    }

    get _tokenHashMapClass(){
        return TokenHashVirtualMapDBModel;
    }

    get _tokenNameMapClass(){
        return TokenNameVirtualMapDBModel;
    }

    get _tokenTickerHashMapClass(){
        return TokenTickerVirtualMapDBModel;
    }

    get _addressTxHashMapClass(){
        return AddressTxHashVirtualMapDBModel;
    }

    get _txInfoHashMapClass(){
        return TxInfoHashVirtualMapDBModel;
    }

    get _txRevertInfoHashMapClass(){
        return TxRevertInfoHashVirtualMapDBModel;
    }

    get _blockHashMapClass(){
        return BlockHashVirtualMapDBModel;
    }

    get _blockHeightMapClass(){
        return BlockHeightVirtualMapDBModel;
    }

    get _accountHashMapClass(){
        return AccountHashVirtualMapDBModel;
    }
    ...
    async _computeGrindingLockedTransfersFundsHeight(height){
        ...
        for (const tx of txs)
            if (tx instanceof SimpleTransactionDBModel ) {
